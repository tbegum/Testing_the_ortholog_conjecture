...
## This function is written to estimate phylogenetic signals (with Blomberg's K and with Pagel's lambda methos) of gene trees
...
## This function helps to identify contrasts standardized trees without using caper R package, so a bit faster
## We can also exclude old nodes of the trees (by specifying "nodelimit") from diagnostic the diagnosnic test analyses
## If limit == 0, we do not exclude any nodes
...
## This function is written to extract phylogenetic indepedent contrasts of speciation events for real or randomized trees 
## frame refers to the dataframe and  x should is the column name (ex: 'pic' or 'pic_abs_random' for real or randomized data))
...
##This function is written to extract phylogenetic indepedent contrasts of duplication events for real or randomized trees
## frame refers to the dataframe and  x should is the column name (ex: 'pic' or 'pic_abs_random' for real or randomized data))
...
##This function is written to extract phylogenetic indepedent contrasts of young duplication events for real or randomized trees
## frame refers to the dataframe and  x should is the column name (ex: 'pic' or 'pic_abs_random' for real or randomized data))
...
##This function is written to extract phylogenetic indepedent contrasts of old duplication events for real or randomized trees
## frame refers to the dataframe and  x should is the column name (ex: 'pic' or 'pic_abs_random' for real or randomized data))
...
## Function tperform two-sided Wilcoxon test on branch transformed trees
...
##This function is written to generate boxplot
...
## This function is written to calculate the proportion of duplication (according to age), speciation and NA events for model fitted trees
...
## Otherwise duplication nodes with the same label name of speciation nodes will be calibrated using the speciation time points or will interfere with the time calibration process  
...
## This function calibrates gene trees based on the speciation time points
...
#### This function is written to paint tree using plotSimmap R function into three different states (speciation, duplication and NA)
...
#### This function is written to paint tree using plotSimmap R function into four different states (speciation, old duplication, young duplication and NA)
paint_tree_mod_new<-function(tree,event)
{
  ...
  ## Checking for Real or randomized data events
  if(event == "Empirical")
  {
...
## Functions required for plotting painted trees
...
## This function is written for model fitting on all trees 
## We used diagnostic tests included in the fitted models to avoid poor model choice
## We considered model with highest AICc weight as the support for the best fit model
model_fitting_genetree_for_Tau<-function(input_tree,trait)
{
  ...
  ## Trait after permutation of tip data 
  if(trait=="rTau")
  {
  ...
  ## Generating dataframe for using OUwie package
  trait <- Tau
  tip<-rownames(trait)
  ...
  ### If the estimated parameter is available in both the "mvMORPH" and "OUwie" packages, we check first with mvMORPH package
...
## This function is written to extract parameters from fitted OU1 model of OUwie
OUwie_OU1_parameters<-function(input_tree)
{
  ...
  ## If only the new duplication events are present, not old
  if(Young_Duplication_node_length > 0 && Old_Duplication_node_length == 0)
  {
    return(tibble(alpha_speciation=as.numeric(df$alpha[1]),
                  alpha_duplication_young=as.numeric(df$alpha[1]),
  ...
  ## If only the old duplication events are present, not new
  if(Young_Duplication_node_length == 0 && Old_Duplication_node_length > 0)
  {
    return(tibble(alpha_speciation=as.numeric(df$alpha[1]),
...
## This function is written to extract parameter values from fitted OU1 model of mvMORPH
...
## This function is written to extract parameter values from fitted OUM model of OUwie
...
## This function is written to extract data from fitted OUM model of mvMORPH
...
## This function is written to extract parameter values from fitted BM1 model of OUwie
...
## This function is written to extract parameters from fitted BM1 model of mvMORPH
...
## This function is written to extract data from fitted BMM model of OUwie
...
## This function is written to extract parameters from fitted BMM model of mvMORPH
...
## This function is written to add duplication, speciation, and NA events
...
## We here used a posterior probability cutoff >0.1 to detect a shift
## In the manuscript, we consider a statistical support for regime shift in the posterior probability >= 0.7
...
##### This function is written to calculate proportions of adaptive regime shifts for speciation and duplication events in the trees
